...
export function AuthShader({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const uniforms = {
      time: { value: 0 },
      uColor: { value: new THREE.Color(isDark() ? 0.6 : 0.3, isDark() ? 0.6 : 0.3, isDark() ? 0.6 : 0.3) },
    };
    ...
    const mo = new MutationObserver(() => {
      const dark = isDark();
      const v = dark ? 0.6 : 0.35;
      uniforms.uColor.value.setRGB(v, v, v);
    });
